...
public class SolicitacaoCompraItemDbg {
	...
	private List<SolicitacaoRatCCustoDbg> rateios;
	private SdcvObc sdcvObc;
	...
	public List<SolicitacaoRatCCustoDbg> getRateios() {
		return rateios;
	}

	public void setRateios(List<SolicitacaoRatCCustoDbg> rateios) {
	...
	public String getCentroCustoObc() {
		...
		if (getFRT_TABCAR_Codigo() != null && !getFRT_TABCAR_Codigo().isEmpty()) {
			...
			return getSTG_GEN_TABEMP_Codigo().toString().concat("-").concat(getGEN_TABCEN_Codigo());
		} else {
			return "";
		}
	...
	public SdcvObc getSdcvObc() {
		return sdcvObc;
	}

	public void setSdcvObc(SdcvObc sdcvObc) {
		this.sdcvObc = sdcvObc;
	}
